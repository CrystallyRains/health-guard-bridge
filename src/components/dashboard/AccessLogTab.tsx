        ...
        <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setViewSummaryIdx(null)}>
          <div className="glass-card p-6 max-w-lg w-full space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-sm">
              <div><span className="text-muted-foreground">Allergies:</span> {patient.allergies.join(", ")}</div>
              <div><span className="text-muted-foreground">Medications:</span> {patient.medications.join(", ")}</div>
              <div><span className="text-muted-foreground">Conditions:</span> {patient.conditions.join(", ")}</div>
